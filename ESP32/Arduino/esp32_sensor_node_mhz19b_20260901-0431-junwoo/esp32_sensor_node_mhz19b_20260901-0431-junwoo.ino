...
// Default off so 10 Hz mmWave TCP can stay up. Press 'u' then 'c' for thermal.
volatile bool thermalUdpEnabled = false;
volatile bool thermalCaptureEnabled = false;
...
void printDiagnosticHelp() {
  ...
  Serial.println(
      "[help] thermal UDP/capture start OFF. Press 'u' then 'c' to enable. "
...
void setup() {
  ...
  Serial.printf(
      ...
      "frame_stale_ms=%lu thermal_udp_default=0\n",
      PIN_MMWAVE_RX, PIN_MMWAVE_TX, static_cast<unsigned long>(MMWAVE_BAUD),
