...
## 2026-09-01 04:30 yuna 변형

- 원본 `esp32_sensor_node_mhz19b_v2` 스케치는 변경하지 않고, 별도 Arduino 스케치 폴더 `esp32_sensor_node_mhz19b_v2_09.01_4-30_yuna`에 추가했습니다.
- 펌웨어 식별자는 `safenest-esp32-sensor-node/1.7.4-mhz19b.2-yuna`, diagnostic build는 `mhz19b-phase-parser-resync-udp-reclaim-20260901-02-yuna`입니다.
- 최신 main의 10 Hz phase JSON 계약과 `breath_rate_raw`를 그대로 유지합니다.
- Seeed v1.0.0의 고착 가능한 partial-frame 상태 대신 bounded UART2 parser를 사용합니다. 잘못된 길이, checksum/type 실패 후 SOF 탐색으로 복귀하고 `frames_ok`, `rejected`, `discarded`, `parser_resync`을 health에 기록합니다.
- Thermal UDP `ENOMEM` 발생 시 연속 실패 10회를 기다리지 않고 UDP socket을 즉시 회수합니다. TCP critical 구간 때문에 첫 회수가 건너뛰어지면 `reclaim_pending`을 유지해 성공할 때까지 Thermal 송신보다 먼저 재시도합니다.
- MH-Z19B UART1 GPIO32/33, 9600 baud, 200 ms timeout, 5초 event 주기, 180초 preheat는 변경하지 않았습니다.
- 이 변형을 사용하려면 해당 스케치 폴더에 실제 `secrets.h`를 넣고 ESP32 WROOM을 업로드 속도 115200으로 다시 플래시해야 합니다.
