...
constexpr char NODE_FIRMWARE_VERSION[] =
    "safenest-esp32-sensor-node/1.7.4-mhz19b.2-yuna";
constexpr char DIAGNOSTIC_BUILD_ID[] =
    "mhz19b-phase-parser-resync-udp-reclaim-20260901-02-yuna";
...
class SafeNestMR60BHA2 : public SEEED_MR60BHA2 {
 public:
  bool handleType(uint16_t type, const uint8_t *data,
    ...
    if (!payloadLengthValid(type, data, dataLength)) return false;
    if (!SEEED_MR60BHA2::handleType(type, data, dataLength)) return false;
    ...
    if (type == static_cast<uint16_t>(
                    TypeHeartBreath::TypeHeartBreathPhase)) {
      phaseTimestampMs_ = millis();
      ++phaseSequence_;
      if (phaseSequence_ == 0) ++phaseSequence_;
    }
    return true;
  }
  ...
  // The Seeed v1.0.0 parser keeps partial-frame state in function-static
  // storage. A corrupt but in-range length can grow past its target forever,
  // leaving UART2 full while update(0) admits no new report. This bounded pump
  // owns resettable state and returns to SOF search after every rejected frame.
  bool update(uint32_t timeout = 0) {
    (void)timeout;
    bool handled = false;
    while (available() > 0) {
      const int next = read();
      ...
      if (frameLength_ >= sizeof(frame_)) {
        ...
        ++parserResyncs_;
        ++rejectedFrames_;
        if (byte == SOF_BYTE) frame_[frameLength_++] = byte;
        continue;
      }
      frame_[frameLength_++] = byte;

      if (frameLength_ == SIZE_FRAME_HEADER) {
        const size_t dataLength =
            (static_cast<size_t>(frame_[3]) << 8) | frame_[4];
        if (dataLength > FRAME_BUFFER_SIZE) {
          resetFrame();
          ++parserResyncs_;
          ++rejectedFrames_;
          continue;
        }
        expectedLength_ = SIZE_FRAME_HEADER + dataLength + SIZE_DATA_CKSUM;
      }
      if (expectedLength_ != 0 && frameLength_ == expectedLength_) {
        if (processFrame(frame_, frameLength_)) {
          handled = true;
        } else {
          ++parserResyncs_;
          ++rejectedFrames_;
  ...
  uint32_t parserResyncs() const { return parserResyncs_; }
  ...
  uint32_t phaseTimestampMs() const { return phaseTimestampMs_; }
  uint32_t phaseSequence() const { return phaseSequence_; }

 private:
  bool payloadLengthValid(uint16_t rawType, const uint8_t *data,
                          size_t dataLength) {
    const TypeHeartBreath type = static_cast<TypeHeartBreath>(rawType);
    switch (type) {
      case TypeHeartBreath::TypeHeartBreathPhase:
        return dataLength >= 3 * sizeof(float);
      case TypeHeartBreath::TypeBreathRate:
      case TypeHeartBreath::TypeHeartRate:
        return dataLength >= sizeof(float);
      case TypeHeartBreath::TypeHeartBreathDistance:
        return dataLength >= sizeof(uint32_t) + sizeof(float);
      case TypeHeartBreath::ReportHumanDetection:
        return dataLength >= 1;
      case TypeHeartBreath::ReportFirmware:
        return dataLength >= sizeof(uint32_t);
      case TypeHeartBreath::Report3DPointCloudDetection:
      case TypeHeartBreath::Report3DPointCloudTargetInfo: {
        if (dataLength < sizeof(uint32_t)) return false;
        const uint32_t targetCount = extractU32(data);
        constexpr size_t TARGET_BYTES =
            2 * sizeof(float) + 2 * sizeof(int32_t);
        return targetCount <= MAX_TARGET_NUM &&
               dataLength >= sizeof(uint32_t) + targetCount * TARGET_BYTES;
      }
      default:
        return false;
    }
  ...
  uint8_t frame_[SIZE_FRAME_HEADER + FRAME_BUFFER_SIZE + SIZE_DATA_CKSUM] = {};
  size_t frameLength_ = 0;
  size_t expectedLength_ = 0;
  uint32_t parserResyncs_ = 0;
  uint32_t rejectedFrames_ = 0;
  uint32_t acceptedFrames_ = 0;
  uint32_t discardedBytes_ = 0;
  uint32_t phaseTimestampMs_ = 0;
  uint32_t phaseSequence_ = 0;
};
...
volatile bool udpReclaimPending = false;
...
void pollMmWave(uint32_t now) {
  ...
  for (uint8_t n = 0; n < 16; ++n) {
    ...
    if (mmWave.getHeartBreathPhases(nextTotalPhase, nextBreathPhase,
      ...
      lastPhaseMs = mmWave.phaseTimestampMs();
      phaseSequence = mmWave.phaseSequence();
  ...
  if (gotPhase || gotBreath || gotHeart) {
    ...
    if (lastMmwSampleLogMs == 0 ||
      ...
      Serial.printf(
          "[mmw] phase=%s breath_phase=%.6f phase_seq=%lu breath=%.2f "
          "heart=%.2f presence=%s uart=%u frames_ok=%lu rejected=%lu "
          "discarded=%lu parser_resync=%lu\n",
          gotPhase ? "fresh" : "cached", breathPhase,
          static_cast<unsigned long>(phaseSequence), respirationRate, heartRate,
          lastPresenceMs == 0 ? "unknown"
                              : (humanDetectedRaw ? "true" : "false"),
          static_cast<unsigned>(mmWaveSerial.available()),
          static_cast<unsigned long>(mmWave.acceptedFrames()),
          static_cast<unsigned long>(mmWave.rejectedFrames()),
          static_cast<unsigned long>(mmWave.discardedBytes()),
          static_cast<unsigned long>(mmWave.parserResyncs()));
    }
  }
}
...
void thermalUdpTask(void *parameter) {
  ...
  for (;;) {
    ...
    // Keep thermal TX stopped until an ENOMEM-triggered rebuild actually runs.
    // A TCP critical section may skip the first attempt; retry after holdoff
    // instead of returning to the same failed socket forever.
    if (udpReclaimPending) {
      const uint32_t reclaimNow = millis();
      if (udpHoldoffActive(reclaimNow)) {
        vTaskDelay(pdMS_TO_TICKS(10));
        continue;
      }
      if (reclaimUdpHeap(udp, udpStarted)) {
        udpReclaimPending = false;
        udpSocketRestarts = udpSocketRestarts + 1;
        Serial.printf(
            "[udp-restart] pending ENOMEM rebuild completed restarts=%lu "
            "reclaim=%lu\n",
            static_cast<unsigned long>(udpSocketRestarts),
            static_cast<unsigned long>(heapReclaims));
      } else {
        requestUdpHoldoff(reclaimNow, UDP_ENOMEM_BACKOFF_MIN_MS);
        vTaskDelay(pdMS_TO_TICKS(10));
        continue;
      }
    }
      ...
      switch (result) {
        ...
        case ThermalSendResult::Failed: {
          ...
          if (err == ENOMEM || err == ENOBUFS || isTransientSendErrno(err)) {
            // ENOMEM cannot use the ten-consecutive-failure guard because this
            // branch resets that counter. Rebuild immediately, then retain
            // exponential holdoff so queued Wi-Fi/lwIP buffers can drain.
            consecutiveHardFailures = 0;
            if (err == ENOMEM) udpReclaimPending = true;
            if (err == ENOMEM && reclaimUdpHeap(udp, udpStarted)) {
              udpReclaimPending = false;
              udpSocketRestarts = udpSocketRestarts + 1;
              Serial.printf(
                  "[udp-restart] socket rebuilt immediately after ENOMEM "
                  "restarts=%lu reclaim=%lu\n",
                  static_cast<unsigned long>(udpSocketRestarts),
                  static_cast<unsigned long>(heapReclaims));
            }
...
void logHealth(uint32_t now) {
  ...
  Serial.printf(
      ...
      "frames_ok=%lu rejected=%lu discarded=%lu parser_resync=%lu\n",
  ...
  Serial.printf(
      ...
      "preheat=%d uart_ppm=%u event_id=%lu accepted=%lu csum_fail=%lu "
      "timeout=%lu short=%lu txn=%s uart1=%u\n",
      co2InPreheat(now) ? 1 : 0, static_cast<unsigned>(co2LastUartPpm),
  ...
  Serial.printf(
      ...
      "reclaim_skip_tcp=%lu reclaim_pending=%d holdoff=%d backoff_ms=%lu "
      "write_to=%lu\n",
      static_cast<unsigned long>(ESP.getFreeHeap()),
      static_cast<unsigned long>(ESP.getMinFreeHeap()),
      static_cast<unsigned long>(ESP.getMaxAllocHeap()),
...
void printDiagnosticHelp() {
  ...
  Serial.println(
      ...
      "cap_on=0 udp_on=0, cap_on=1 udp_on=0, cap_on=1 udp_on=1. Watch [link] "
      "write_max_ms, gap_max_ms, udp_errno, heap, max_alloc, holdoff.");
}
