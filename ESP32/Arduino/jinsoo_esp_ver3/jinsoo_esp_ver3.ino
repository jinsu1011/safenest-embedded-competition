...
// MR60BHA2 는 호흡/심박 프레임을 명령 없이 스스로 스트리밍합니다. 이 노드는
// 레이더에 아무것도 쓰지 않으므로 TX 를 구동할 이유가 없습니다.
//
// 그런데 구동하면 해롭습니다. ESP32 가 리셋(EN, 재플래시, 브라운아웃)될 때
// GPIO17 이 순간적으로 LOW 로 글리치하고, MR60 은 이를 UART BREAK 로 읽어
// 부트로더/업데이트 모드로 들어갑니다. 그 상태에서는 애플리케이션으로 못 넘어가
// 0x0A13(HeartBreathPhase)을 아예 내지 않습니다.
//
// 현장 증거: MR60 UART 를 그대로 덤프하면 아래가 0.7초마다 반복되었습니다.
//   "Andar ADT6101P people counting V1.6.12"
//   "[Boot Area] isUpdateFg = 0x2, updateAppxFg: 0x4, loadFg: 0x2"
//   "[Boot Area] Flash read done."
// isUpdateFg=0x2 는 업데이트 대기 플래그입니다.
//
// false 면 UART2 초기화 직후 GPIO17 을 입력(고임피던스)으로 되돌려 레이더의
// RX 를 절대 구동하지 않습니다. 배선을 물리적으로 빼는 것과 같은 효과입니다.
constexpr bool MMWAVE_TX_ENABLED = false;
// MR60 이 자체 부팅을 마칠 시간. 이 전에 UART 를 만지면 부팅을 방해합니다.
constexpr uint32_t MMWAVE_BOOT_SETTLE_MS = 1500;
...
// Thermal 프레임 반복 전송 횟수.
//
// 한 프레임은 9개 chunk 이고 Pi 는 9개가 모두 도착해야 완성합니다. 즉
// datagram 하나만 잃어도 그 프레임은 통째로 사라집니다. 현장 측정에서
// 완성 850 / 미완성 144 = 약 14% 프레임 손실이었는데, datagram 단위로는
// 약 1.7% 입니다. 1.7% 손실이 9번 반복되면 프레임 성공률이 86% 로 떨어지는
// 구조적 취약점입니다.
//
// UDP 에는 재전송이 없으므로 같은 프레임을 한 번 더 보냅니다. Pi 의
// ThermalUDPReassembler 는 이미 중복 chunk 를 정상 처리하므로(duplicate_chunks
// 로 세고 무시) 프로토콜은 바뀌지 않습니다.
//
// 비용: 1 fps 기준 약 86 -> 172 kbps. 무선이 나쁘면 1 로 낮추십시오.
constexpr uint8_t THERMAL_UDP_FRAME_REPEAT = 2;
...
void thermalUdpTask(void *parameter) {
  ...
  for (;;) {
    ...
    if (xQueueReceive(thermalQueue, &thermalNetworkFrame, 0) == pdTRUE) {
      ...
      ThermalSendResult result = sendThermalUdp(udp, thermalNetworkFrame);
      // 첫 전송이 성공했을 때만 한 번 더 보냅니다. 실패/보류 상태에서 반복하면
      // 이미 막힌 무선을 더 막습니다. 반복분 결과는 통계에 다시 넣지 않습니다
      // (프레임 수를 부풀리지 않기 위해).
      for (uint8_t repeat = 1;
           repeat < THERMAL_UDP_FRAME_REPEAT && result == ThermalSendResult::Sent;
           ++repeat) {
        vTaskDelay(pdMS_TO_TICKS(UDP_CHUNK_GAP_MS));
        (void)sendThermalUdp(udp, thermalNetworkFrame);
      }
...
void logAiReadiness(uint32_t now) {
  ...
  if (phaseSequence == 0 && mmWave.framesParsed() > 50) {
    diagPrintf(
        "[ai]     진단: 프레임은 %lu개 파싱됐는데 phase 이벤트는 0 입니다. "
        "레이더가 호흡/심박(0x0A13) 이 아닌 다른 타입만 보내고 있습니다. "
        "MR60 이 부트로더/사람수세기 모드로 부팅한 상태입니다. "
        "MR60 전원만 껐다 켜보세요(ESP 는 그대로).\n",
        static_cast<unsigned long>(mmWave.framesParsed()));
  }
  if (phaseSequence == 0) {
    Serial.println(
...
void setup() {
  ...
  // MR60 이 스스로 부팅을 끝낸 뒤에 UART 를 건드립니다.
  setupWait(MMWAVE_BOOT_SETTLE_MS);
  mmWaveSerial.setPins(PIN_MMWAVE_RX, PIN_MMWAVE_TX);
  mmWave.begin(&mmWaveSerial, MMWAVE_BAUD, 0);
  if (!MMWAVE_TX_ENABLED) {
    // UART2 초기화가 끝난 뒤 TX 패드만 고임피던스로 되돌립니다. 수신은 그대로
    // 동작하고, 레이더의 RX 는 이제 아무도 구동하지 않습니다.
    pinMode(PIN_MMWAVE_TX, INPUT);
    Serial.printf(
        "[mmw] TX(GPIO%d) 비활성화 - 레이더 RX 를 구동하지 않습니다. "
        "BHA2 는 명령 없이 스트리밍하므로 수신만으로 충분하고, ESP 리셋 시 "
        "글리치가 레이더를 부트로더로 밀어넣는 경로를 없앱니다.\n",
        PIN_MMWAVE_TX);
  }
