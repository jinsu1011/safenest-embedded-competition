...
5. `SafeNest_V4_OnDevice_AI/docs/MR60_INTEGRATION.md`
...
`SafeNest_V4_OnDevice_AI/adapters/run_mr60_serial_adapter.py`의 실제 포트 입력과 `integrated_node/safenest_risk_engine.py`의 소비 경로를 확인한다. replay가 아니라 실제 ESP JSONL을 사용해 다음을 증명한다.
...
/private/tmp/safenest-pi-regression-venv/bin/python -m unittest \
  SafeNest_V4_OnDevice_AI/tests/test_mr60_esp_adapter.py \
  SafeNest_V4_OnDevice_AI/tests/test_mmwave_stream_adapter.py \
  SafeNest_V4_OnDevice_AI/tests/test_mmwave_input_adapter.py \
  SafeNest_V4_OnDevice_AI/tests/test_mr60_manifest.py -v
```
