...
- [ ] XIAO ESP32-C6 연결 후 현재 MR60 펌웨어 버전과 Seeed 라이브러리 버전 기록
- [ ] 무필터 기준선 수집: 빈 공간 5분
- [ ] 무필터 기준선 수집: 가슴 정면 0.8~1.0m 안정 1인 5분
- [ ] 무필터 기준선 수집: 진입→정지→퇴장 20회 및 ground truth 기록
- [ ] 원본 로그를 보존하고 기준선 지표 계산
- [ ] 동일 원본 로그에서 필터 또는 유효성 조건을 한 번에 하나씩 비교
- [ ] 실측 근거로 ESP 상태기계와 텔레메트리 구현
