...
void loop() {
  ...
  if (isFrameReady) {
    ...
    // ----------------------------------------
    // UDP 데이터 쪼개서 전송 (MTU 제한 우회)
    // ----------------------------------------
    // 한 번에 10KB를 UDP로 쏘면 공유기(MTU 1500)에서 드랍될 확률이 매우 높으므로 1440바이트 단위로 쪼개서 보냅니다.
    const int chunkSize = 1440;
    uint8_t* byteBuffer = (uint8_t*)frameBuffer;
    int bytesSent = 0;
    
    while (bytesSent < TOTAL_BYTES) {
      int currentChunk = min(chunkSize, TOTAL_BYTES - bytesSent);
      udp.beginPacket(receiverIP, receiverPort);
      udp.write(byteBuffer + bytesSent, currentChunk);
      udp.endPacket();
      bytesSent += currentChunk;
      delay(1); // 버퍼 오버플로우 방지용 아주 짧은 딜레이
    }

    Serial.println("Frame sent via UDP (Chunked, 10,080 Bytes total).");
  }
}
