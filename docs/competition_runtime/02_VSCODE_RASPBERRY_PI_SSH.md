...
1. 노트북과 Raspberry Pi를 같은 Wi-Fi `EELab04 2G`에 연결합니다.
2. VS Code를 설치합니다.
...
ZIP을 Raspberry Pi로 옮기는 쉬운 방법은 VS Code 원격 창의 Explorer로 파일을 드래그하거나, 노트북 PowerShell에서 `scp`를 사용하는 것입니다.

```powershell
scp SafeNest_GitHub_Package.zip sandi@192.168.0.50:~/
```

Raspberry Pi 터미널에서 압축을 풉니다.

```bash
cd ~
unzip SafeNest_GitHub_Package.zip
```

프로젝트 안의 두 실행 폴더를 홈으로 복사합니다.

```bash
cp -a ~/SafeNest_GitHub_Package/raspberry_pi_lcd ~/
cp -a ~/SafeNest_GitHub_Package/SafeNest_Web ~/
```
