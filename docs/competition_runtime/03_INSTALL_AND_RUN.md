...
지원 기준은 Raspberry Pi OS 64-bit와 Node.js 18 이상입니다. 프로젝트 루트에서 실행합니다.

```bash
cd ~/SafeNest_GitHub_Package
...
1. 노트북 Wi-Fi를 `EELab04 2G`로 맞춥니다.
...
패키지를 홈 폴더의 `~/SafeNest_GitHub_Package`에 두었다면 다음으로 두 서버를 함께 실행할 수 있습니다.
