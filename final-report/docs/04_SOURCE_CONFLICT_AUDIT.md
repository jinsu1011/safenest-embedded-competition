...
| **C12** | **CO₂ 임계값** | 유승하 표: NORMAL 400–600 / CAUTION >1,000 / WARNING ≥1,500 / EMERGENCY ≥2,000 — 모두 "**팀 내부 기준, 기관 출처 없음**", LCD 서버에는 **미구현** | Risk 엔진에는 상수 존재: `co2_ppm > 1500` (`risk_engine.py` L146), `>= 2000` (fallback rule) | 코드에는 있으나 **대외 근거 없음** | "내부 실험 임계값"으로만 표기 + "**공식 기준 추가 확인 필요**". 공식 안전기준으로 제시 금지 |
