...
class SafeNestRiskEngine:
    ...
    def calculate_quality_gate(self, packet: dict):
        ...
        q_mmwave = 1.0 if (
            ...
            and mmwave_data.get("state", "VALID") == "VALID"
            and mmwave_data.get("presence", 1) == 1
            and mmwave_data.get("breath_valid", True) is True
            ...
            and float(mmwave_data.get("breath_rpm")) > 0.0
        ) else 0.0
    ...
    def evaluate_risk(self, packet: dict):
        ...
        presence_val = mmwave_data.get("presence") if isinstance(mmwave_data, dict) else None
        presence_confirmed = presence_val == 1

        if isinstance(mmwave_data, dict) and q_gate["mmwave"] == 1.0:
            apnea = mmwave_data.get("apnea")
            apnea_verified = mmwave_data.get("apnea_verified") is True
            breath_rpm = mmwave_data.get("breath_rpm")
            heart_bpm = mmwave_data.get("heart_bpm")
            resp_phase_val = mmwave_data.get("resp_phase")
            model_allowed = mmwave_data.get("model_allowed") is True
            ...
            if self.mmwave_runner is not None and model_allowed and mm_fallback_reason is None:
            ...
            if not model_allowed:
                mm_ai_status = "DISABLED_UNVERIFIED"
                mm_fallback_reason = "MMWAVE_MODEL_DISABLED_UNVERIFIED"
            ...
            resp_eval = self.rules_evaluator.evaluate_respiration(
                breath_rpm, apnea, mm_class, valid=True,
                apnea_verified=apnea_verified,
                sample_timestamp=sample_ts,
            )
            hr_valid = (
                mmwave_data.get("heart_valid") is True
                and heart_bpm is not None and np.isfinite(heart_bpm)
                and 20.0 <= heart_bpm <= 240.0
            )
            hr_eval = self.rules_evaluator.evaluate_vital_hr(
                heart_bpm,
                valid=hr_valid,
                verified=mmwave_data.get("heart_verified") is True,
            )
        else:
            resp_eval = self.rules_evaluator.evaluate_respiration(None, None, valid=False, sample_timestamp=sample_ts)
            heart_bpm = mmwave_data.get("heart_bpm") if isinstance(mmwave_data, dict) else None
            hr_eval = self.rules_evaluator.evaluate_vital_hr(
                heart_bpm,
                valid=isinstance(mmwave_data, dict) and mmwave_data.get("heart_valid") is True,
                verified=isinstance(mmwave_data, dict) and mmwave_data.get("heart_verified") is True,
            )
            mm_ai_status = "DEGRADED"
            mm_fallback_reason = (
                mmwave_data.get("fault_reason")
                if isinstance(mmwave_data, dict) and mmwave_data.get("fault_reason")
                else (
                    "MMWAVE_PRESENCE_NOT_DETECTED"
                    if presence_val == 0 else "RESP_SENSOR_FAULT"
                )
            )
            if presence_val == 0:
        ...
        elif isinstance(mmwave_data, dict) and mmwave_data.get("breath_source"):
            mm_source = mmwave_data.get("breath_source")
            mm_fallback = True
            mm_reason = mm_fallback_reason or "MMWAVE_MODEL_DISABLED_UNVERIFIED"
        else:
            mm_source = "none"
            mm_fallback = True
        ...
        return {
            "risk_score": float(v4_fusion.risk_score),
            ...
            "derived_metrics": {
                ...
                "mmwave_state": mmwave_data.get("state") if isinstance(mmwave_data, dict) else None,
                "breath_rpm": mmwave_data.get("breath_rpm") if isinstance(mmwave_data, dict) else None,
                "breath_rpm_raw": mmwave_data.get("breath_rpm_raw") if isinstance(mmwave_data, dict) else None,
                "breath_source": mmwave_data.get("breath_source") if isinstance(mmwave_data, dict) else None,
                "breath_confidence": mmwave_data.get("breath_confidence") if isinstance(mmwave_data, dict) else 0.0,
                "heart_bpm": mmwave_data.get("heart_bpm") if isinstance(mmwave_data, dict) else None,
                "heart_valid": mmwave_data.get("heart_valid") is True if isinstance(mmwave_data, dict) else False,
                "heart_verified": mmwave_data.get("heart_verified") is True if isinstance(mmwave_data, dict) else False,
                "heart_confidence": mmwave_data.get("heart_confidence") if isinstance(mmwave_data, dict) else 0.0,
            },
            "model_meta": {
                "thermal": {
