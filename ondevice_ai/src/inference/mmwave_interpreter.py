...
class MMWaveInterpreter:
    def __init__(
        ...
        if self.model_file_exists and tflite is not None:
            try:
        ...
        elif not self.model_file_exists:
            self.load_error_reason = "TFLITE_MODEL_FILE_MISSING"
        else:
            self.load_error_reason = "TFLITE_RUNTIME_NOT_INSTALLED"
    ...
    def predict(self, window: np.ndarray) -> MMWavePrediction:
        ...
        if self.interpreter is not None:
            ...
            # A missing/unloadable model must never manufacture APNEA from a
            # flat or missing signal. Callers treat every fallback as UNKNOWN.
            probabilities = np.array([0.34, 0.33, 0.33], dtype=np.float32)
            fallback_used = True
            fallback_reason = self.load_error_reason or "MMWAVE_MODEL_UNVERIFIED_FALLBACK"
