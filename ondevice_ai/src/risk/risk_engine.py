...
"""
risk/risk_engine.py
SafeNest V4 Multi-Sensor Risk Fusion Engine
...
class RiskEngineV4:
        ...
        for channel in ["S1", "S2", "S3", "S4"]:
            ...
            if norm_val is not None:
                ...
                if last_ts is not None and (now - last_ts) <= ttl and channel in self.last_good_scores:
                    ...
                    if (
                        raw_inputs and channel == "S1"
                        and raw_inputs.get("apnea_verified") is True
                        and self._normalize(raw_inputs.get("apnea")) == 1.0
                    ):
                        scores[channel] = 1.0
    ...
    def evaluate_packet(
        ...
        if s1 is not None and self._normalize(s1) is not None:
            ...
            if (
                isinstance(mm, dict) and mm.get("valid", True)
                and mm.get("breath_valid", True)
                and self._normalize(mm.get("breath_rpm")) is not None
            ):
                channel_scores["S1"] = 1.0 if (
                    mm.get("apnea") == 1 and mm.get("apnea_verified") is True
                ) else 0.0
            else:
...
class SafeNestRiskEngine:
    def __init__(
        ...
        self.fallback_engine = FallbackEngine(weights=self.weights)
    ...
    def evaluate(
        ...
        active_keys = set(k for k, v in sensor_results.items() if v and v.valid)

        stale_found = False
        for k, v in sensor_results.items():
            if v and v.valid and (current_time - v.timestamp > 3.0):
                stale_found = True
                break

        if expected_keys != active_keys or stale_found:
            return self.fallback_engine.evaluate_fallback(sensor_results, now=current_time)

        s1 = sensor_results["mmwave"].score
        s2 = sensor_results["co2"].score
        s3 = sensor_results["pir"].score
        s4 = sensor_results["thermal44"].score

        if not all(np.isfinite([s1, s2, s3, s4])):
            return self.fallback_engine.evaluate_fallback(sensor_results, now=current_time)
        ...
        if s4 == 1.0:
            r_score = 100.0
            is_emergency = True
            reasons.append("EMERGENCY_HUMAN_FALL")

        if (
            s1 == 1.0
            and sensor_results["mmwave"].metadata.get("apnea_verified") is True
        ):
            r_score = 100.0
            is_emergency = True
            reasons.append("EMERGENCY_HARDWARE_APNEA")

        if not is_emergency:
            r_val = (
                self.weights["mmwave"] * s1 +
                self.weights["co2"] * s2 +
                self.weights["pir"] * s3 +
                self.weights["thermal44"] * s4
            )
            r_score = float(np.clip(r_val * 100.0, 0.0, 100.0))

        if r_score >= self.caution_max:
            level = "DANGER"
        elif r_score >= self.normal_max:
            level = "CAUTION"
        else:
            level = "NORMAL"
