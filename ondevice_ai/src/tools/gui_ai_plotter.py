...
class SafeNestGUIPlotter:
    def __init__(self, model_path="safenest_lstm_quant.tflite", metadata_path="sensor_stats_metadata.json"):
        if not os.path.exists(model_path) or not os.path.exists(metadata_path):
            raise FileNotFoundError("모델 파일 또는 메타데이터 파일이 존재하지 않습니다. 경로를 확인하세요.")
