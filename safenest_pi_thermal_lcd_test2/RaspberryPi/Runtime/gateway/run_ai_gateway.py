#!/usr/bin/env python3
"""Run receiver, state manager, and PHASE 5 AI evaluation as one process."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="SafeNest receiver + state + on-device AI")
    ...
    try:
        while not stopping:
            print(json.dumps(pipeline.evaluate(), ensure_ascii=False, allow_nan=False), flush=True)
