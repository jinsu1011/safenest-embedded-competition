...
MMWAVE_DEGRADED_REASONS = {
    ...
    "MMWAVE_WARMUP",
    "MMWAVE_MODEL_DISABLED_UNVERIFIED",
    "MODEL_APNEA_CANDIDATE_UNVERIFIED",
    "UNVERIFIED_APNEA_IGNORED",
}
...
class RiskRulesEvaluator:
    def __init__(self, config_path: str | Path | None = None) -> None:
        if config_path is None:
            ...
            config_path = project_root / "config" / "risk_engine.json"
        else:
        ...
        self.last_resp_timestamp: float | None = None
    ...
    def evaluate_respiration(
        ...
        valid: bool = True,
        apnea_verified: bool = False,
        dt_s: float | None = None,
        sample_timestamp: float | None = None
    ) -> RuleResult:
        if (
            not valid or not _is_finite_number(breath_rpm)
            or float(breath_rpm) <= 0.0 or apnea not in (0, 1, None)
        ):
        ...
        if sample_timestamp is not None:
            ts_valid, ts_reason = validate_timestamp(sample_timestamp, previous=self.last_resp_timestamp)
            if not ts_valid:
                self.apnea_started_at = None
                self.apnea_timer_s = 0.0
                return RuleResult(score=0.5, reasons=[ts_reason or "SENSOR_TIMESTAMP_INVALID"], status="FAULT")
            self.last_resp_timestamp = float(sample_timestamp)

        if apnea == 1 and apnea_verified:
            return RuleResult(
                score=1.0,
        ...
        self.apnea_started_at = None
        self.apnea_timer_s = 0.0
        if apnea == 1 and not apnea_verified:
            return RuleResult(
                score=0.5,
                reasons=["UNVERIFIED_APNEA_IGNORED"],
                status="DEGRADED",
            )
        if ai_class == 2:
            return RuleResult(
                score=0.5,
                reasons=["MODEL_APNEA_CANDIDATE_UNVERIFIED"],
                status="DEGRADED",
            )
    ...
    def evaluate_vital_hr(
        ...
        if not verified:
            return RuleResult(score=0.0, reasons=["HR_UNVERIFIED"], status="DEGRADED")
    ...
    def evaluate_system(
        ...
        sensor_status = {
            ...
            "vital_hr": "DEGRADED" if (
                "HR_SENSOR_MISSING" in vital_hr_eval.reasons
                or "HR_UNVERIFIED" in vital_hr_eval.reasons
            ) else "OK",
        }
